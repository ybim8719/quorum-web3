...
const LotsList = ({ lots, totalLotsShares, onLink }: ILotProps) => {
        ...
        tableBody = lots.map((l, i) => {
            return (
                <tr key={`customer-${i}`}>
                    ...
                    <td className="">{l.customer ? `${l.customer.firstName} ${l.customer.lastName}` : "N/A"}</td>
                    <td className="">{l.customer ? `${l.customer.address.slice(0, 15)}...` : "N/A"}</td>
                    <td>{l.customer == undefined ? <button className="nes-btn is-success" onClick={() => onLink(l.id)}>Link to a customer</button> : "N/A"}</td>
                </tr>
            );
        });
    } else {
    ...
    return (
        <div className="section">
            ...
            <p><u>Current Total Shares: {totalLotsShares}</u></p>
            <table>
                <thead>
                    <tr>
